 ...
 *
 * Afficheur 7 segments personnalisable avec gestion étendue :
 * - Affiche un chiffre (`digit`) ou un caractère (`char`) compatible 7 segments
 * - Supporte aussi l’allumage manuel des segments via `manualSegments`
 * - Inclut des effets visuels réalistes (biseau, glow, flicker)
 * - Possède un mode idle animé affichant une séquence répétée
 *
 * @param modifier Modificateur de mise en page standard Jetpack Compose
 * @param digit Chiffre de 0 à 9 à afficher (prioritaire sur `char` si défini)
 * @param char Caractère à afficher (doit être compatible 7 segments, ex: 'A', 'E', 'P', etc.)
 * @param manualSegments Liste booléenne de 7 éléments pour contrôler manuellement les segments (écrase `digit` et `char`)
 * @param segmentLength Longueur des segments verticaux (côtés F/E/C/B)
 * @param segmentHorizontalLength Longueur des segments horizontaux (A/D/G)
 * @param segmentThickness Épaisseur des segments
 * @param bevel Taille du biseau sur les coins pour un rendu plus réaliste
 * @param onColor Couleur des segments allumés
 * @param offColor Couleur des segments éteints
 * @param alpha Transparence globale des segments (entre 0f et 1f)
 * @param glowRadius Rayon du flou pour simuler un effet lumineux type LED
 * @param flickerAmplitude Intensité de l’effet de scintillement
 * @param flickerFrequency Fréquence du scintillement (en Hz)
 * @param idleMode Active l’animation idle si true (remplace l’affichage courant)
 * @param idleSpeed Vitesse de rotation des segments pendant le mode idle (en ms)
 */
@Composable
fun SevenSegmentDigitDisplayExtended(
